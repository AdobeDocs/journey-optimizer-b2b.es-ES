...
description: Aprenda a utilizar los nodos Acción externa y recorrido de rutas de división externa para conectarse con servicios externos y enrutar cuentas y personas en función de la respuesta del servicio.
...
Utilice nodos externos para conectar el recorrido con un servicio externo. Cuando una audiencia llega a uno de estos nodos, [!DNL Journey Optimizer B2B Edition] envía asincrónicamente datos de atributo de audiencia al servicio externo. El servicio procesa los datos y responde con una llamada de retorno, devolviendo la información de audiencia y los metadatos que el recorrido utiliza para continuar.

>[!NOTE]
>
>Un administrador debe [configurar y activar la acción externa](../admin/configure-external-actions.md) para que los especialistas en marketing puedan agregar e implementar estos nodos en un recorrido.
...
* **[Rutas divididas externas](#external-split-paths)**: llama a un servicio externo y evalúa la respuesta para enrutar cuentas o personas a lo largo de una de varias rutas definidas. Utilice este nodo cuando el servicio externo devuelva un valor, como una puntuación o nivel, que determina el siguiente paso del recorrido.
...
1. Navegue hasta el lienzo del recorrido de cuenta o persona.
...
1. (Solo recorridos de cuenta) En las propiedades del nodo a la derecha, establezca la **[!UICONTROL Acción en]** contexto para la acción externa:
...
El nodo Rutas de acceso divididas externas llama a un servicio externo y utiliza la respuesta para determinar qué ruta siguen las cuentas o personas. Una condición basada en una variable (descriptor de acceso) devuelta por el servicio externo define cada ruta. El recorrido evalúa la respuesta con respecto a las condiciones de ruta definidas y enruta cada cuenta o persona a lo largo de la primera ruta coincidente. Las condiciones de ruta se evalúan en orden descendente. Cada cuenta o persona continúa con la primera ruta cuya condición coincida con el valor devuelto por el servicio externo.
...
1. (Solo recorridos de cuenta) En las propiedades del nodo a la derecha, elija un tipo **[!UICONTROL Dividir rutas por]**:
